...
- **`✅ DONE`** ~~Tool logos fill their holder.~~ The gaps were **in the source images**, not the CSS — 16 of a 30-icon sample filled under 90% of their own canvas (worst: 22%). `iconUrl()` in `tools.js` appends ImageKit's `tr=t-true:w-128,h-128,c-at_max` to trim the padding server-side.
