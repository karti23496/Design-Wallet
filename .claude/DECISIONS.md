...
- **`DECIDED` `.nav-dropdown--panel` keeps `position: relative`; only `--wide` goes static.** `--wide` anchors to the whole header because a 1120px panel has to. A 600px panel centred that way would float away from the word it belongs to, so this one anchors to its own trigger with `left: 0` and a `max-width: calc(100vw - 32px)` guard. **The rules must sit after the base `.nav-dropdown-menu` hover rule** — equal specificity, and the base one's `translate(-50%)` wins otherwise. Verified on screen at **1440, 1280, 1100 and 1000px**: the panel is 600px, two columns, fully on screen at every width, and a **20-step pointer walk from the trigger to the first card never closes it** (it carries its own `::before` bridge, like `--wide`). At 390px it stacks to one column inside the sheet, indistinguishable from Mini Tools.
